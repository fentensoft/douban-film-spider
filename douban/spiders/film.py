...
class FilmSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        yield scrapy.Request(url="https://movie.douban.com/tag/?view=cloud", meta={"proxy": self.proxy}, callback=self.parse_tags)

    def parse_tags(self, response):
        hrefs = response.xpath('//table[@class="tagCol"]//a/@href').extract()
        for i in hrefs:
            yield scrapy.Request(url="https://movie.douban.com" + i, meta={"proxy": self.proxy}, cookies={"bid": "".join(random.sample(string.ascii_letters + string.digits, 20))}, callback=self.parse_pages)
    ...
    def parse_pages(self, response):
        ...
        if len(lists) > 0:
            ...
            for i in range(last_page):
                yield scrapy.Request(url=baseurl + str(20 * i), meta={"proxy": self.proxy}, cookies={"bid": "".join(random.sample(string.ascii_letters + string.digits, 20))}, callback=self.parse_list)
        else:
    ...
    def parse_list(self, response):
        ...
        for i in hrefs:
            if i not in self.f:
                self.f.add(i)
                yield scrapy.Request(url=i, meta={"proxy": self.proxy}, cookies={"bid": "".join(random.sample(string.ascii_letters + string.digits, 20))}, callback=self.parse_film)
